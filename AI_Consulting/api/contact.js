// Vercel Serverless Function for AI Consulting Contact Form
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const recipientEmail = process.env.CONTACT_EMAIL || process.env.AI_CONTACT_EMAIL;
    const emailService = process.env.EMAIL_SERVICE || 'resend'; // 'resend', 'sendgrid', or 'nodemailer'

    if (!recipientEmail) {
      console.error('CONTACT_EMAIL or AI_CONTACT_EMAIL environment variable not set');
      return res.status(500).json({ error: 'Email configuration missing' });
    }

    // Prepare email content
    const subject = `New Contact Form Submission - AI Consulting`;
    const emailBody = `
New contact form submission from AI Consulting website:
...
---
This message was sent from the AI Consulting contact form.
    `.trim();
    ...
    if (emailService === 'resend') {
      emailSent = await sendWithResend(recipientEmail, subject, emailBody, { name, email, message });
    } else if (emailService === 'sendgrid') {
      emailSent = await sendWithSendGrid(recipientEmail, subject, emailBody, { name, email, message });
    } else {
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: `
          ...
          <hr>
          <p><em>This message was sent from the AI Consulting contact form.</em></p>
        `,
      }),
    });
    ...
    const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        from: { 
          ...
          name: 'AI Consulting Contact Form'
        },
        subject: subject,
        content: [
          {
